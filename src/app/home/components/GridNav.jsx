...
const GridNav = ({categories, selectedcategory}) => {
  return (
    ...
    <ul className='w-[90%] mx-auto max-w-5xl flex justify-around text-black mb-12' id='instaGrid'>
        <li className='flex justify-between items-center uppercase text-sm lg:text-xl'>
        ...
        {categories.map(category => (
            <li className='flex justify-between items-center' key={category}>
                ...
                <a className={`uppercase text-sm lg:text-xl ${selectedcategory==category?'font-semibold':''}`} href={`/${category}#instaGrid`}>{category}</a>
            </li> 
        ))}
    </ul>
  )
